...
DELETE FROM osm_landusages_gen0 WHERE ST_GeometryType(geometry) IN ('ST_Point', 'ST_Linestring', 'ST_MultiPoint', 'ST_MultiLinestring');


-- Suppression des composantes des géométries non surfaciques de osm_landusages_gen0
with geom_comps as (
...
DELETE FROM osm_landusages_gen0
WHERE osm_id IN (
	SELECT osm_id from not_polygon_objects
	)
;

UPDATE osm_landusages_gen0
SET geometry = new_geoms.geometry
FROM new_geoms
WHERE new_geoms.osm_id = osm_landusages_gen0.osm_id
;

-- Changement de type de géométrie de osm_landusages_gen0
ALTER TABLE osm_landusages_gen0
