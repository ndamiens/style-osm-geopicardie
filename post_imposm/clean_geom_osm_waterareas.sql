...
DELETE FROM osm_waterareas WHERE ST_GeometryType(geometry) IN ('ST_Point', 'ST_Linestring', 'ST_MultiPoint', 'ST_MultiLinestring');


-- Suppression des composantes des géométries non surfaciques de osm_waterareas
with geom_comps as (
...
FROM new_geoms
WHERE new_geoms.osm_id = osm_waterareas.osm_id
;

-- Changement de type de géométrie de osm_waterareas
ALTER TABLE osm_waterareas
